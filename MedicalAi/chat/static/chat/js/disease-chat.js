...
function loadDiseaseData() {
    // Check if the URL is defined in the global scope
    if (typeof window.DISEASE_JSON_URL !== 'undefined' && window.DISEASE_JSON_URL) {
        console.log('Loading disease data from:', window.DISEASE_JSON_URL);
        
        fetch(window.DISEASE_JSON_URL)
            .then(response => {
                if (!response.ok) {
            ...
            .catch(error => {
                ...
                // Try to load from a fallback path if available
                if (window.DISEASE_JSON_URL.includes('data/24-Disease.json')) {
                    const fallbackUrl = window.DISEASE_JSON_URL.replace('data/24-Disease.json', '24-Disease.json');
                    console.log('Trying fallback URL:', fallbackUrl);
                    fetch(fallbackUrl)
                        .then(r => r.json())
                        .then(d => {
                            console.log('Successfully loaded disease data from fallback URL');
                            diseaseData = d;
                        })
                        .catch(e => console.error('Error loading from fallback URL:', e));
                }
            });
    } else {
        console.error('DISEASE_JSON_URL is not defined in the global scope');
        console.log('Available globals:', Object.keys(window).filter(k => k === 'DISEASE_JSON_URL' || k === 'TOPICS_JSON_URL' || k === 'API_URLS'));
    }
}

// Load disease data when the script loads
loadDiseaseData();
...
function processDiseaseResponse(predictions) {
    try {
        console.log('Processing disease response with predictions:', predictions);
        ...
        const diseaseName = topPrediction.name;
        
        // If disease data is not loaded yet, try to load it
        if (diseaseData.length === 0) {
            console.log('Disease data not loaded yet, attempting to reload...');
            loadDiseaseData();
            
            // If we still don't have data, show a fallback message
            if (diseaseData.length === 0) {
                const fallbackResponse = `Based on your symptoms, you might have <strong>${diseaseName}</strong> with a confidence of ${(topPrediction.confidence * 100).toFixed(1)}%.`;
                return fallbackResponse + "\n\n<em>Note: Additional disease information is currently unavailable. Please try again in a moment.</em>";
            }
        }
        ...
        window.currentDisease = currentDisease;
        resetShownSections();
...
function formatDiseaseResponse(disease, confidenceScore) {
    return `<div class="disease-info">
        <div class="disease-title">🩺 <strong>${disease.name}</strong> <span class='text-xs text-gray-500'>(Confidence: ${confidenceScore.toFixed(2)}%)</span></div>
        ...
        case 'otherPossibilities':
            if (allPredictions.length > 1) {
                ...
                const possibilitiesContent = alternatives.map((pred, index) => {
                    return `<button class="text-left hover:text-blue-600 dark:hover:text-blue-400 transition-colors duration-200" onclick="handleAlternativePrediction('${pred.name}', ${pred.confidence})">
                        ${index + 2}. ${pred.name} — ${pred.confidence.toFixed(2)}%
                    </button>`;
                }).join('<br>');
    ...
    if (!shownSections.otherPossibilities) {
        ...
        if (allPredictions.length > 1) {
            ...
            possibilitiesContent = alternatives.map((pred, index) => {
                const disease = diseaseData.find(d => d.name.toLowerCase() === pred.name.toLowerCase());
                if (disease) {
                    return `<button class="text-left hover:text-blue-600 dark:hover:text-blue-400 transition-colors duration-200" onclick="handleAlternativePrediction('${pred.name}', ${pred.confidence})">
                        ${index + 2}. ${pred.name} — ${pred.confidence.toFixed(2)}%
                    </button>`;
                }
                return `${index + 2}. ${pred.name} — ${pred.confidence.toFixed(2)}%`;
            }).join('<br>');
        } else {
